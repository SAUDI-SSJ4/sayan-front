    ...
    const fetchCourseData = async () => {
      try {
        ...
        if (response.data) {
          ...
          setError("فشل في تحميل بيانات الدورة - لا توجد بيانات");
        }
      } catch (error) {
        ...
        if (error.response?.status === 404) {
          ...
          setError("يجب تسجيل الدخول للوصول إلى هذه الدورة");
        } else if (error.response?.status === 403) {
          setError("ليس لديك صلاحية للوصول إلى هذه الدورة");
        } else if (error.code === 'NETWORK_ERROR' || error.message.includes('Network Error')) {
          setError("خطأ في الاتصال بالشبكة. الرجاء المحاولة مرة أخرى");
        } else {
          setError(error.response?.data?.message || error.message || "حدث خطأ في تحميل الدورة");
        }
      } finally {
        setLoading(false);
      }
...
هل يمكنك توضيح سؤالك أكثر لأتمكن من مساعدتك بشكل أفضل؟`;
  ...
  if (!courseData) {
    return (
      <div className="d-flex justify-content-center align-items-center" style={{ minHeight: "400px" }}>
        <div className="text-center">
          <h4>لم يتم العثور على الدورة</h4>
        </div>
      </div>
    );
  }
        ...
        <div className={`${classes.content} ${classes.lessonsVideo}`}>
          {!chosenLesson ? (
            <div className="text-center p-5">
              ...
              {courseData?.data?.categories?.length === 0 && (
                <p className="text-muted mt-3">هذه الدورة لا تحتوي على محتوى حالياً</p>
              )}
            </div>
            ...
            <div className={classes.videoContent}>
              <div className={classes.Course}>
                {/* التحقق من وجود فيديو أو محتوى */}
                {(chosenLesson?.video?.id || 
                  chosenLesson?.content?.video?.id || 
                  chosenLesson?.content?.src ||
                  chosenLesson?.video?.stream_url ||
                  chosenLesson?.content?.video ||
                  (chosenLesson?.video && Object.keys(chosenLesson.video).length > 0) ||
                  (chosenLesson?.content && chosenLesson.content.src) ||
                  (chosenLesson?.content && chosenLesson.content.video && chosenLesson.content.video !== null)
                ) ? (
                <div className={classes.videoContainer}>
                  <CustomVideoPlayer video={chosenLesson?.video?.id || chosenLesson?.content?.video?.id} />
                </div>
                ) : (
                  <motion.div 
                    className="text-center p-5" 

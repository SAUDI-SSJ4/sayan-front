...
import { useOpinion } from "../../../framework/accademy/academysetting-opinions";
import { Spinner } from "react-bootstrap";

const EditStudentOpinion = () => {
  const navigate = useNavigate();
  const { id } = useParams();
  const { data: studentOpinion, isLoading } = useOpinion(id);
  return (
    <div className="mb-5 all-info-top-header main-info-top">
      ...
      <div className="CustomCard formCard all-add-notific pb-4 pt-4">
        {isLoading && <Spinner />}
        {!isLoading && studentOpinion && (
          <StudentOpinionForm studentOpinion={studentOpinion.data} />
        )}
      </div>
    </div>
  );
...
export default EditStudentOpinion;

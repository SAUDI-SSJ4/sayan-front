...
import { getCourse, getTrainer, getAbout, getSlider, getAcademySettings, getAllcategories, getCourseSummary } from "../utils/apis/client/academy.js";
...
export const useTrainer = () => {
    ...
    if (isFetched && !isError && data?.length > 0 && (!trainers || trainers.length === 0)) {
...
export const useCategories = () => {
    ...
    const { data: fetchedCategories, isError, isFetched, isLoading } = useQuery({
        queryKey: ["categories"],
    ...
    useMemo(() => {
        if (isFetched && !isError && fetchedCategories?.length > 0 && (!categories || categories.length === 0)) {
...
export const useCourseSummary = (courseId) => {
    const { data: courseSummary, isError, isLoading } = useQuery({

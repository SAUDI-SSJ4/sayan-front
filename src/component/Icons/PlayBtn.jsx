const PlayBtn = ({ 
    color = '#009AFF', // Default blue color
    iconColor = '#009AFF', // Color for the play icon
  }) => {
    return (
      <svg 
      width="42" 
      height="42" 
        viewBox="0 0 42 42" 
        fill="none" 
        xmlns="http://www.w3.org/2000/svg"
      >
        <circle 
          cx="21" 
          cy="21" 
          r="21" 
          fill={color} 
          fillOpacity="0.13"
        />
        <g clipPath="url(#clip0_16_121)">
          <path 
            d="M21 4C11.6111 4 4 11.6111 4 21C4 30.3888 11.6111 37.9999 21 37.9999C30.3888 37.9999 37.9999 30.3888 37.9999 21C37.99 11.6153 30.3847 4.01003 21 4ZM28.1593 21.5416C28.0416 21.7777 27.8503 21.9691 27.6142 22.0868V22.0928L17.8999 26.95C17.3 27.2497 16.5707 27.0065 16.2709 26.4065C16.1856 26.236 16.1417 26.0477 16.1428 25.8571V16.1429C16.1425 14.9283 16.6859 14.928 17.3565 14.928C17.5452 14.9279 17.7312 14.9718 17.8999 15.0561L27.6142 19.9132C28.2144 20.2123 28.4585 20.9414 28.1593 21.5416Z" 
            fill={iconColor}
          />
        </g>
        <defs>
          <clipPath id="clip0_16_121">
            <rect 
              width="34" 
              height="34" 
              fill="white" 
              transform="translate(4 4)"
            />
          </clipPath>
        </defs>
      </svg>
    );
  }
export default PlayBtn;
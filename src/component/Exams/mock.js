...
const myArray = ['طالب', 'موظف سيان', 'اكاديمية تعليمية', 'موظف اكاديمية', 'مدير خدمة العملاء'];
...
export function mockUsers(length) {
  const createRowData = rowIndex => {
    ...
    const user = {
        ...
        name: fakerAR.person.firstName() + ' ' + fakerAR.person.lastName(),
    }
    ...
    const role = faker.helpers.arrayElement(myArray);
    ...
    const isActive=faker.helpers.arrayElement(["active","ended",'not'])
